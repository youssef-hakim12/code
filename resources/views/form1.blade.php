...
@if(session('success'))
    <script>
        Swal.fire({
            ...
            window.location.href = "{{ route('myfatorah') }}"; 
        });
    </script>
@endif
            ...
            <form action="{{ route('form.store') }}" method="POST" id="bookingForm">
                @csrf
                <div class="mb-3">
                    <input type="text" name="job" class="form-control" value="{{ request()->query('button_name') }}" readonly>
                </div>
                <div class="mb-3">
